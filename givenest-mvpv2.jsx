  ...
  return (
    <div>
      ...
      <div style={{ position:"relative", minHeight:"92vh", display:"flex", alignItems:"center", overflow:"hidden" }}>
        ...
        {/* Dark overlay + fallback bg */}
        <div style={{ position:"absolute", inset:0, background:"linear-gradient(to right, rgba(0,0,0,0.75) 0%, rgba(0,0,0,0.5) 60%, rgba(0,0,0,0.25) 100%)" }} />
        {/* Content */}
        <div style={{ position:"relative", zIndex:1, maxWidth:1100, margin:"0 auto", padding:"100px 48px 80px", width:"100%" }}>
          <div style={{ fontSize:11, fontWeight:500, letterSpacing:"0.12em", textTransform:"uppercase", color:"rgba(255,255,255,0.7)", marginBottom:20 }}>Arizona's giving brokerage</div>
          <h1 className="serif" style={{ fontSize:"clamp(38px,5vw,68px)", fontWeight:600, lineHeight:1.1, marginBottom:20, letterSpacing:"-0.02em", color:"#FFFFFF", maxWidth:700 }}>
            Every home funds a cause <em style={{ color:T.coral }}>you choose.</em>
          </h1>
          <p style={{ fontSize:17, fontWeight:300, color:"rgba(255,255,255,0.82)", maxWidth:460, lineHeight:1.8, marginBottom:40 }}>
            Buy or sell with Givenest and we donate to a charity of your choice at closing — at no extra cost.
          </p>
          {/* Search bar */}
          <div style={{ display:"flex", gap:0, maxWidth:580, boxShadow:"0 4px 32px rgba(0,0,0,0.3)", borderRadius:8, overflow:"hidden" }}>
            <input
              value={search}
          ...
          <div style={{ marginTop:18, display:"flex", gap:28, flexWrap:"wrap" }}>
            <button className="btn-ghost" onClick={() => setTab("sell")} style={{ fontSize:13, color:"rgba(255,255,255,0.75)", borderColor:"transparent" }}>Get a selling estimate →</button>
            <button className="btn-ghost" onClick={() => setTab("charities")} style={{ fontSize:13, color:"rgba(255,255,255,0.75)", borderColor:"transparent" }}>Browse charities →</button>
          </div>
        </div>
      </div>
      ...
      <div style={{ background:"#F4F3EE", padding:"80px 32px" }}>
        <div style={{ maxWidth:960, margin:"0 auto" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:16 }}>
            {[
              { icon:"01", title:"Find your home", desc:"Search Arizona listings. See the donation amount on every property." },
              { icon:"02", title:"Choose a charity", desc:"Pick any 501(c)(3) in the country. Your choice is saved to your transaction." },
              { icon:"03", title:"Choose your agent", desc:"Work with any agent. They handle the transaction. We make the donation." },
            ].map((step, i) => (
          ...
          <div style={{ position:"sticky", top:68 }}>
            <div style={{ background:"#FFFFFF", borderRadius:10, padding:26, borderTop:`3px solid ${T.coral}`, border:`1px solid ${T.border}` }}>
              ...
              <div style={{ marginBottom:18 }}>
                {[["List price",fmt(home.price),false],["Brokerage commission (2.5%)",fmt(commission),false],["Givenest donates",fmt(givingPool),true]].map(([l,v,hi]) => (
      ...
      <div style={{ maxWidth:860, margin:"0 auto", padding:"44px 32px", display:"grid", gridTemplateColumns:"1fr 320px", gap:44 }}>
        <div>
          ...
          <div className="card" style={{ padding:24 }}>
            ...
            {num > 0 && (
              <div>
                {[["Home value",fmt(num),false],["Brokerage commission (2.5%)",fmt(commission),false],["Givenest donates",fmt(givingPool),true]].map(([l,v,hi]) => (
