...
export interface BuyClientInitialData {
  listings: Property[];
  pinnedListings: Property[];
  total: number;
  totalPages: number;
  /** IP-derived lat/lng from the RSC shell (if available). Used to seed
   *  userLat/userLng so the client doesn't block the first paint on GPS
   *  or /api/user-location. Client GPS can still refine later. */
  lat: number | null;
  lng: number | null;
}

interface BuyClientProps {
  initial: BuyClientInitialData;
}
...
function BuyPage({ initial }: BuyClientProps) {
  ...
  // If the RSC shell seeded us with initial data, skip the very first client
  // fetch — it would just re-request what the server already rendered. The
  // ref is consumed once on the initial effect run; subsequent filter/sort
  // changes always go through /api/listings like before.
  const hasSeedData = initial.listings.length > 0 || initial.pinnedListings.length > 0;
  const hasHydratedInitialData = useRef(hasSeedData);
  ...
  const [userLat, setUserLat] = useState<number | null>(initial.lat);
  const [userLng, setUserLng] = useState<number | null>(initial.lng);
  const [locationSource, setLocationSource] = useState<"ip" | "gps" | null>(
    initial.lat !== null && initial.lng !== null ? "ip" : null
  );
  // locationReady skips the initial-load gate when: URL has an explicit
  // location filter, OR the RSC shell pre-rendered a first page of listings
  // (in which case we already have data — no need to wait for lat/lng).
  const [locationReady, setLocationReady] = useState(
    () => urlLocation !== null || hasSeedData
  );
  ...
  const [properties, setProperties] = useState<Property[]>(initial.listings);
  const [pinnedListings, setPinnedListings] = useState<Property[]>(initial.pinnedListings);
  const [loading, setLoading] = useState(!hasSeedData);
  const [total, setTotal] = useState(initial.total);
  const [sortBy, setSortBy] = useState(() => searchParams.get("sort") ?? "recommended");
  const [nextPage, setNextPage] = useState(hasSeedData ? 2 : 1);
  const [hasMore, setHasMore] = useState(hasSeedData ? initial.totalPages > 1 : true);
  ...
  useEffect(() => {
    ...
    async function detect() {
      ...
      // Fall back to IP if GPS wasn't available AND the RSC shell didn't
      // already seed us with IP lat/lng (saves a redundant round-trip).
      const alreadyHaveIp = initial.lat !== null && initial.lng !== null;
      if (!gotGps && !alreadyHaveIp && !cancelled) {
        try {
  ...
  const fetchListings = useCallback(async (targetPage: number) => {
    ...
    const isGeoSort = (sortBy === "recommended" || sortBy === "nearest") && userLat !== null;
    params.set("limit", isGeoSort ? "100" : "12");
...
export default function BuyClient({ initial }: BuyClientProps) {
  return (
    <Suspense>
      <BuyPage initial={initial} />
    </Suspense>
  );
}
